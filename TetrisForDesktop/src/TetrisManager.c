...
void TetrisManager_Init(TetrisManager* tetrisManager, int speedLevel){
	...
	Block block;
	block.current = -1;
	_TetrisManager_InitBoard(tetrisManager, speedLevel); /////레벨 별 맵을 보드에 불러오기 위해 speedLevel 추가
...
static void _TetrisManager_InitBoard(TetrisManager* tetrisManager, int speedLevel){
	...
	_TetrisManager_LevelMap(tetrisManager, speedLevel, FIXED_BLOCK); /////레벨 별로 맵을 그리는 함수 실행
}
